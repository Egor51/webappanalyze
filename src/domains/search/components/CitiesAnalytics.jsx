...
import { formatPrice, formatPercent } from '../../../shared/utils/formatters'
...
const CitiesAnalytics = memo(({ data, onBack }) => {
  ...
  // Проверяем данные заранее и мемоизируем
  const cities = useMemo(() => data?.content || [], [data])
  const hasData = cities.length > 0

  // Мемоизированная сортировка городов (всегда вызывается)
  ...
  }, [cities, sortField, sortDirection, hasData])

  // Мемоизированная пагинация (всегда вызывается)
  const { paginatedCities, totalPages, startIndex, endIndex } = useMemo(() => {
    const start = currentPage * itemsPerPage
    const end = start + itemsPerPage
    const paginated = sortedCities.slice(start, end)
    const total = Math.ceil(sortedCities.length / itemsPerPage)
    return { paginatedCities: paginated, totalPages: total, startIndex: start, endIndex: end }
  }, [sortedCities, currentPage, itemsPerPage])

  // Сброс страницы при изменении сортировки (всегда вызывается)
  useEffect(() => {
    setCurrentPage(0)
  }, [sortField, sortDirection])

  // Автофокус на заголовок после загрузки данных (всегда вызывается)
  useEffect(() => {
    if (hasData && headerRef.current) {
      setTimeout(() => {
