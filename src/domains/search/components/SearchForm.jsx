...
import { useAddressSuggestions, useCitySuggestions } from '../hooks'
...
const SearchForm = ({ onSearch, searchType: externalSearchType, onSearchTypeChange, onGetAllCities }) => {
  ...
  // React Query хуки для suggestions
  const addressSuggestionsQuery = useAddressSuggestions({
    query: address,
    enabled: searchType === 'address' && !isSuggestionSelected && address.trim().length >= 2,
  })

  const citySuggestionsQuery = useCitySuggestions({
    query: address,
    enabled: searchType === 'city' && !isSuggestionSelected && address.trim().length >= 2,
  })

  // Определяем активный query в зависимости от типа поиска
  const activeSuggestionsQuery = searchType === 'address' 
    ? addressSuggestionsQuery 
    : searchType === 'city' 
    ? citySuggestionsQuery 
    : null

  // Получаем suggestions из query
  const suggestions = activeSuggestionsQuery?.data || []
  const isLoadingSuggestions = activeSuggestionsQuery?.isLoading || false

  // Обновляем показ suggestions при изменении данных
  useEffect(() => {
    if (searchType === 'district') {
  ...
  }, [suggestions, searchType, isSuggestionSelected, showSuggestions])
