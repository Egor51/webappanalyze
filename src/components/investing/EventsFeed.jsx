...
const EventsFeed = ({ isPro = false, mandates = [] }) => {
  ...
  const filteredEvents = filter === 'all' 
    ? events 
    : events.filter(e => {
        if (filter === 'price') return e.type === EVENT_TYPES.PRICE_DROP || e.type === EVENT_TYPES.PRICE_INCREASE
        if (filter === 'match') return e.type === EVENT_TYPES.NEW_MATCH
        if (filter === 'listing') return e.type === EVENT_TYPES.LONG_LISTING
        if (filter === 'volume') return e.type === EVENT_TYPES.VOLUME_SPIKE
        if (filter === 'pattern') return e.type === EVENT_TYPES.PATTERN
        return true
      })
