 ...
 */
export const calculateRentalPotential = (option, _analyticsResponse) => {
...
export const calculateInterestLevel = (option, flipPotential, rentalPotential) => {
  ...
  if (option.differencePercent) {
    if (option.differencePercent < -20) {
score += 2
} else if (option.differencePercent < -10) {
score += 1.5
} else if (option.differencePercent < -5) {
score += 1
} else if (option.differencePercent > 10) {
score -= 1
}
  ...
  if (flipPotential && flipPotential.isProfitable) {
    if (flipPotential.flipMarginPercent > 30) {
score += 2
} else if (flipPotential.flipMarginPercent > 20) {
score += 1.5
} else if (flipPotential.flipMarginPercent > 10) {
score += 1
}
  }
  ...
  if (rentalPotential) {
    if (rentalPotential.rentalYield > 12) {
score += 1
} else if (rentalPotential.rentalYield > 8) {
score += 0.5
}
  }
